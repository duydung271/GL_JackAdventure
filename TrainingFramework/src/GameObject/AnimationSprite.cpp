...
(std::shared_ptr<Models> model, std::shared_ptr<Shaders> shader, std::shared_ptr<Texture> texture, int numFrames, float frameTime)
	:Sprite2D(model,shader,texture)
	, m_numFrames(numFrames), m_frameTime(frameTime)
	,m_currentFrame(0), m_currentTime(0.0f)
{}
...
void AnimationSprite::UpdateType2(GLfloat deltatime, bool& allowOverFrameStop, bool &isStopAni, int sttFrameStop)
{
	m_currentTime += deltatime;
	if (m_currentTime > m_frameTime) {
...
void AnimationSprite::SetFrameTime(float newFrameTime)
{
