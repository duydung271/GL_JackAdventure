...
{
	Matrix m_Sc, m_T, m_Rx,m_Rz;
	m_Sc.SetScale(m_Vec3Scale);
	m_Rx.SetRotationX(m_Vec3Rotation.x);
	m_Rz.SetRotationZ(m_Vec3Rotation.z);
	m_T.SetTranslation(m_Vec3Position);
	m_WorldMat = m_Sc *m_Rx*m_Rz* m_T;
}

void Sprite2D::SetRotation(float angle)
{
	m_Vec3Rotation = Vector3(0, 0, angle * PI / 180.0f);
	Init();
}
